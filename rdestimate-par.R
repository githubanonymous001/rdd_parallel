#
# main rdd function, modified for parallel execution 
#
...
RDestimate.par<-function(formula, data, subset=NULL, cutpoint=NULL, bw=NULL, kernel="triangular", se.type="HC1", cluster=NULL, verbose=FALSE, model=FALSE, frame=FALSE) {
  ...
  for(bw in bws){
    ...
    if(type=="sharp"){
      ...
      if (is.null(cluster)) {
        debug_vcovHC <- vcovHC(mod,type=se.type)
      ...
      if (is.null(cluster)) {
        ######################################################################################
        #Code change to call new version of vcovHC
        debug_vcovHC <- vcovHC_par(mod,type=se.type)
        o$se[ibw]<-coeftest(mod,debug_vcovHC)[2,2]
        ######################################################################################
      } else {
        o$se[ibw]<-robust.se(mod,cluster[na.ok][w>0])
